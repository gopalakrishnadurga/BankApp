...
public class BankAppTestCases {
	...
	@Test
	public void test_withdrawMethod_with_valid() throws InsufficientOpeningBalanceException {
		Account account=new Account();
		account.setAccountNo(123);
		account.setCustomer(customer);
		...
		Mockito.when(accountDao.withdraw(account, 5000.0)).thenReturn(account1);
	//Account account2=accountService.withdraw(account,4000.0);
	
//	Mockito.verify(accountDao).findAccount(123);
	Mockito.verify(accountDao).withdraw(account,  5000.0);
	
	//assertEquals(2000.0, account1.getOpeningBalance(),0.0);
	}
